...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		if (beanDefinition instanceof AbstractBeanDefinition) {
			try {
				 ...
				 * 主要是对AbstractBeanDefinition属性中 methodOverrides 进行校验，
		...
		//如果未存在
			...
			//检测创建 Bean 阶段是否已经开启，如果开启了则需要对 beanDefinitionMap 进行并发控
	...
	public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
		...
		//javaxInjectProviderClass 类注入的特殊处理
			...
			//为实际依赖关系目标的延迟解析构建代理
			//默认实现返回 null
			//在属性或set方法上使用了@Lazy注解，那么则构造一个代理对象象并返回，真正使用该代理对象时才进行类型筛选Bean
			Object result = getAutowireCandidateResolver().getLazyResolutionProxyIfNecessary(
					descriptor, requestingBeanName);
			if (result == null) {
				//通用处理逻辑
				//descriptor表示某个属性或某个set方法
				//requestingBeanName表示正在进行依懒注入的Bean
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			//支持 Spring 的注解 @value 获取@Value所指定的值
